...
public class NewsInfoAction extends ActionSupport {
	...
	public String update() {
		...
		if (admin == null) {
			...
			news = new NewsInfo(newsInfoId, name, summary, content, service
					.searchNewsInfo(newsInfoId).getNewsInfoTime(), author,
					admin.getAdminId(), type.toString(), newsInfoState);// 创建时间不变
			*/
			
			String type = newsType[0];
			setMsg(MessageUtil.get("newsinfo.update.false"));
			newsInfoId = pid;
			
			news = new NewsInfo(newsInfoId, name, summary, content, service
					.searchNewsInfo(newsInfoId).getNewsInfoTime(), author,
					admin.getAdminId(), type, newsInfoState);// 创建时间不变
	...
	public String insert() {
		...
		if (admin == null) {
			...
			newsInfoId = ((infoList.size() == 0) ? 1 : (infoList.get(infoList.size()-1).getNewsInfoId()+1));// 新的ID等于最大的ID加1

			newsInfoState = 1;
			news = new NewsInfo(newsInfoId, name, summary, content, new Date(
					new java.util.Date().getTime()), author,
					admin.getAdminId(), type.toString(), newsInfoState);// 创建时间为当前时间
			*/
		...
		if (admin == null) {
			...
			news = new NewsInfo(newsInfoId, name, summary, content, new Date(
					new java.util.Date().getTime()), author,
					admin.getAdminId(), type, newsInfoState);// 创建时间为当前时间
	...
	public String list() {
		...
		try {
			...
			List<NewsInfo> all = service.getAllNewsInfo(keyWord, currentPage,
					lineSize);
